                            ...
                            {user.map((obj, index) => {
                                return (
                                    <tr>
                                        ...
                                        <td>{obj.email}</td>
                                        <td>{obj.mobile}</td>
                                        <td><Button variant="success" onClick={() => { editUser(obj._id, obj.name, obj.email, obj.mobile) }} style={{ marginRight: 10,display:'none' }}>Edit</Button>
                                            <Button variant="danger" onClick={() => { if (window.confirm('Do you want to delete this user?')) { deleteUser(obj._id) } }} style={{display:'none'}}>Delete</Button></td>
